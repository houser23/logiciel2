...
<SCRIPT>
	...
	*/
	function convert1 () {
		...
		if (columnNames.indexOf("potentiel_fiscal") < 0 && columnNames.indexOf("dp_personnels") < 0 && columnNames.indexOf("fondssolidarite_versement") < 0 && columnNames.indexOf("dp_apa_etablissement") < 0 && columnNames.indexOf("dp_apa_domicile") < 0 && columnNames.indexOf("Dp_apa") < 0 && columnNames.indexOf("DepTotales") < 0 && columnNames.indexOf("base_foncier") < 0 && columnNames.indexOf("fondssolidarite_prelevement") < 0 && columnNames.indexOf("dp_apa") < 0 && columnNames.indexOf("cvae") < 0 && columnNames.indexOf("dp_investissement") < 0 && columnNames.indexOf("cnsa_apa") < 0 && columnNames.indexOf("cnsa_mdph") < 0 && columnNames.indexOf("cnsa_pch") < 0 && columnNames.indexOf("dp_fonctionnement") < 0 && columnNames.indexOf("epargne_gestion") < 0 && columnNames.indexOf("dp_pch") < 0 && columnNames.indexOf("dgf") < 0 && columnNames.indexOf("forfaitaire") < 0 && columnNames.indexOf("compensation")< 0 && columnNames.indexOf("dpu")< 0 && columnNames.indexOf("dfm")< 0 && columnNames.indexOf("dmto")< 0 && columnNames.indexOf("epargne_brute")< 0 && columnNames.indexOf("epargne_nette") < 0 && columnNames.indexOf("interets_financiers") < 0 && columnNames.indexOf("remboursement_capital") < 0 && columnNames.indexOf("endettement") < 0 && columnNames.indexOf("vfdmto") < 0 && columnNames.indexOf("rsa") < 0 && columnNames.indexOf("racsansmecanisme") < 0 && columnNames.indexOf("recettes_investissement") < 0 && columnNames.indexOf("dp_apa") < 0 && columnNames.indexOf("soldefondssolidarite") < 0 && columnNames.indexOf("recettes_fonctionnement") < 0 && columnNames.indexOf("vcvae") < 0 && columnNames.indexOf("pfdmto") < 0 && columnNames.indexOf("dgd") < 0 && columnNames.indexOf("ecretement") < 0 && columnNames.indexOf("fngir") < 0 && columnNames.indexOf("pcvae") < 0 && columnNames.indexOf("dcrtp") < 0 && columnNames.indexOf("dcrtp_ecart") < 0 && columnNames.indexOf("rac") < 0) {
		...
		results.forEach(function(row, index) {
			...
			if (row.recettes_investissement)
				results[index].recettes_investissement = Math.round(results[index].recettes_investissement / results[index].population * 100 ) / 100 ;
			if (row.recettes_fonctionnement)
				results[index].recettes_fonctionnement = Math.round(results[index].recettes_fonctionnement / results[index].population * 100 ) / 100 ;
			if (row.vcvae)
				results[index].vcvae = Math.round(results[index].vcvae / results[index].population * 100 ) / 100 ;
			if (row.dgd)
				results[index].dgd = Math.round(results[index].dgd / results[index].population * 100 ) / 100 ;
			if (row.fngir)
				results[index].fngir = Math.round(results[index].fngir / results[index].population * 100 ) / 100 ;
			if (row.pfdmto)
				results[index].pfdmto = Math.round(results[index].pfdmto / results[index].population * 100 ) / 100 ;
			if (row.pcvae)
...
<div style="display: flex; flex-direction: row; justify-content: space-around; flex-wrap: wrap">
	...
	<div style="display: flex; flex-direction: column; margin: 5px 10px; align-items: center;">
		...
		<select name="d1" size =5 multiple="multiple"> 
			...
			<option = value="ecretement"> Ecrêtement </option>
			...
			<option = value="dcrtp_ecart"> Ecart DCRTP </option>
...
<a href="donchr1bisplusieursechelle.php"><button style="width:85px;
	height:85px;
	background:#fafafa;
